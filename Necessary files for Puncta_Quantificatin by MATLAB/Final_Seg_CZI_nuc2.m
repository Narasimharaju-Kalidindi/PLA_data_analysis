...
squeezebi = squeeze(image6d(1,1,:,2,:,:));
...
figure, imshow(squeeze(Ai(1,:,:))), colorbar,caxis([0 2000]),title(['Original Master Image',num2str(i)])
saveas(figure(2*i-1),fullfile(DESTINATION_FOLDER,['1_Original_Master_Image',num2str(i),'.png']))
%%%%%%%%%%
disp('Generating original master image')
figure, imshow(squeeze(BAbz(1,:,:))), colorbar,caxis([0 2000]),title(['Original Nuclei Image',num2str(i)])
